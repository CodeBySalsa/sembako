...
<div x-data="{ 
    ...
    <div class="bg-green-50 rounded-2xl p-3 shadow-sm border border-green-100 flex flex-col cursor-pointer relative" 
            ...
            <div class="max-w-4xl w-full grid md:grid-cols-2 gap-10 bg-white p-8 rounded-[2rem] shadow-2xl border border-green-100" @click.away="detailOpen = false">
                ...
                <div class="flex flex-col justify-center">
                    ...
                    <div class="flex gap-3">
                        <button type="button" 
                                @click.stop="$store.cart.add({
                                    nama: detailItem.nama, 
                                    finalPrice: (detailItem.kategori === 'Beras') ? (detailItem.harga * (selectedWeight / 5)) : detailItem.harga,
                                    gambar: detailItem.gambar,
                                    weight: (detailItem.kategori === 'Beras') ? selectedWeight : 0,
                                    qty: 1
                                });" 
                                class="flex-1 bg-emerald-600 text-white py-3 rounded-xl font-bold text-sm hover:bg-emerald-700 transition shadow-md">
                            + Keranjang
                        </button>
                        <button type="button" 
                                @click="
                                    let msg = 'Halo, saya ' + $store.profile.nama + ' dari ' + $store.profile.alamat + 
                                              '. Ingin memesan ' + detailItem.nama + (detailItem.kategori === 'Beras' ? ' (' + selectedWeight + 'kg)' : '') + '. Kontak: ' + $store.profile.no_hp;
                                    window.open('https://wa.me/083196633554?text=' + encodeURIComponent(msg), '_blank');
                                "
                                class="flex-1 bg-green-500 text-white py-3 rounded-xl font-bold text-sm hover:bg-green-600 transition shadow-md">
                            WhatsApp
                        </button>
                    </div>
                </div>
